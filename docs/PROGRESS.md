...
## 1. Trạng thái tổng quan (cập nhật 2026-07-28, phiên bản **v166**)

- **✨ (v166) test: cổng chặn "không vỡ giao diện trên mobile" + fix 1 lỗi thật cổng bắt được.**
  User quyết định KHÔNG làm PWA (xem lý do trong bộ nhớ tự động), chốt yêu cầu thay thế: mọi
  thiết kế UI phải dùng được trên mobile, không vỡ giao diện. Quy tắc này đã có trong
  `docs/UIUX_DESIGN.md` nhưng đã vỡ 2 lần trước đây (v127, v129 — phải sửa 28 file thiếu
  `grid-cols-1` ở base) vì lỗi tràn ngang **không nhìn thấy được** khi phát triển trên màn hình
  rộng, chỉ lộ ra khi có người mở bằng điện thoại. Biến quy tắc thành cổng chặn máy móc thay
  vì chỉ dựa "nhớ kiểm tra".
  - `frontend/e2e/mobile-khong-vo.spec.js` (mới): khổ điện thoại 375×700, duyệt **mọi trang
    chính của cả 3 vai trò** (HS qua thanh tab đáy màn hình, GV/Admin qua drawer hamburger),
    fail nếu `document.documentElement.scrollWidth > clientWidth` — dấu hiệu chuẩn của tràn
    ngang. Chạy tự động trong job e2e sẵn có (không cấu hình gì thêm), nên tự nằm trong hook
    chặn trước khi push. **Không bắt được** chữ quá nhỏ/nút sát nhau — vẫn cần mắt người cho
    việc đó, đây chỉ là lưới an toàn cho đúng lớp lỗi đã từng xảy ra.
  - **🐞 Cổng chặn bắt được NGAY 1 lỗi thật, đang LIVE trên production từ v162**: trang
    "Nhật ký" của Admin (`admin/NhatKy.jsx`) sập trắng (`TypeError: rows.map is not a
    function`). Nguyên nhân: v162 đổi shape trả về của `listSessionsHoanThanh()` từ mảng
    phẳng sang `{rows, tong}` (phân trang server-side) — đã sửa đúng chỗ gọi ở
    `gv/TheoDoiTienBo.jsx` nhưng **sót chỗ gọi thứ hai** ở `admin/NhatKy.jsx`. Grep xác nhận
    đây là chỗ sót DUY NHẤT, đã sửa `setHoanThanh(h)` → `setHoanThanh(h.rows)`.
  - Cập nhật nhãn mô tả job e2e ở cả `scripts/kiem-tra-truoc-khi-push.ps1` và `ci.yml` (từ
    "3 luồng vàng" thành "luồng vàng + cổng chặn mobile") — CLAUDE.md mục 2a yêu cầu 2 nơi
    này không lệch nhau.
  - Build-test-fix: `eslint` sạch · `vitest` 38/38 · `vite build` OK · `pytest` 653/653
    (không đụng backend) · **e2e Playwright 7/7** (4 luồng vàng cũ + 3 ca cổng chặn mobile
    mới, mỗi ca duyệt 7-13 trang).
  - **Kiểm chứng trên app thật (dev.db)**: trang Nhật ký Admin trước đây sập trắng, sau khi
    sửa hiển thị đúng bảng dữ liệu.

## 1a. Trạng thái trước đó (v165)

- **🐞 (v165) fix: bấm Back của trình duyệt sau khi rời phòng học ra TRANG TRẮNG.** User báo
  sau khi hoàn thành bài → bấm "Bài nên luyện tiếp" → sang Chọn bài đã lọc → bấm Back thì
  phòng học trắng trơn.
  - **Bug CÓ SẴN TỪ TRƯỚC, không phải do v163** — đã kiểm chứng bằng nút "Chọn bài khác" (có
    từ lâu): Back cũng trắng y hệt. v163 chỉ làm nó dễ gặp hơn hẳn vì thêm một nút rời phòng
    học ngay màn hình hoàn thành. Mọi đường rời phòng học đều dính.
  - **Nguyên nhân**: rời phòng học → `onHashChange`/`dieuHuong` xóa `sessionStorage`
    `hs_phong_hoc` + `setPhongHoc(null)`. Back đưa hash về `#phong_hoc` nhưng không còn phiên
    nào để mở, mà JSX là `{page === 'phong_hoc' && phongHoc && <PhongHoc/>}` → **render rỗng**.
  - **Sửa 3 lớp:**
    1. **Giữ** `hs_phong_hoc` khi rời trang (bỏ `removeItem` ở cả `dieuHuong` và
       `onHashChange`) — lịch sử trình duyệt vẫn có `#phong_hoc` nên phải còn phiên để quay lại.
    2. `onHashChange` khi về `#phong_hoc` thì **đọc lại phiên từ sessionStorage** (state đã bị
       null lúc rời trang trước đó).
    3. **Lưới an toàn**: `page === 'phong_hoc'` mà không có phiên → điều hướng về Chọn bài,
       TUYỆT ĐỐI không render trắng. Bọc mọi đường còn lại (gõ tay hash, F5, link cũ).
    + Dọn `hs_phong_hoc` lúc **đăng xuất** (key giờ sống lâu hơn nên phải dọn, tránh HS kế
      tiếp đăng nhập cùng tab còn sót phiên của người trước).
  - **🐞 Lỗi thứ hai lộ ra nhờ chính test e2e vừa viết** (assertion đỏ, không phải đoán): khi
    mở bài bằng "Bắt đầu", sessionStorage chỉ lưu `{problemId}` — Back khôi phục problemId rồi
    `createSession()` **tạo phiên MỚI** (phiên cũ đã `hoan_thanh` nên backend không tái dùng),
    tức Back làm HS **bắt đầu lại bài từ đầu** thay vì thấy kết quả vừa xong. Sửa: `PhongHoc`
    báo `session_id` thật qua `onSid` → `ghiNhanSid()` ghi đè sessionStorage thành
    `{sessionId}`. **CHỈ ghi sessionStorage, KHÔNG `setPhongHoc`** — prop `key` của PhongHoc
    tính từ `phongHoc.sessionId/problemId`, đổi state ở đây sẽ ép remount và tạo vòng lặp vô tận.
  - Build-test-fix: `eslint` sạch · `vitest` 38/38 · `vite build` OK · `pytest` 653/653 (không
    đụng backend) · **e2e Playwright 4/4** — thêm ca regression ngay trong luồng vàng #1: hoàn
    thành bài → "Chọn bài khác" → Back → phải thấy lại khung hoàn thành.
  - **Kiểm chứng thủ công 5 kịch bản trên app thật**: Back sau "Bài nên luyện tiếp" (đúng ca
    user báo) · Back sau "Chọn bài khác" (chứng minh bug có sẵn) · gõ tay `#phong_hoc` không có
    phiên · F5 thẳng vào `#phong_hoc` không có phiên · Back rồi Forward. Tất cả đều không còn
    trắng.

## 1b. Trạng thái trước đó (v164)

- **✨ (v164) feat: cờ `khong_phan_tich_duoc` — tự báo GV khi CAS không đọc được biểu thức HS
  nhập ≥ ngưỡng lần trong 1 phiên.** Bối cảnh: rà nhận định bài dự thi "khi chưa đủ căn cứ,
  hệ thống không kết luận mà chuyển GV xem xét" thì thấy vế sau CHƯA đúng nguyên văn — lúc CAS
  trả `KHONG_PHAN_TICH_DUOC`, hệ thống chỉ mời HS nhập lại, **không** báo ai cả. Cờ này lấp
  đúng khoảng đó. Giá trị thật: lặp lại nhiều lần thường là dấu hiệu `bieu_thuc_ket_qua` của
  bước bị hỏng — CAS sẽ KHÔNG BAO GIỜ chấm đúng dù HS nhập chuẩn tuyệt đối, lỗi âm thầm chỉ lộ
  ra khi có người xem.
  - **🐞 Vá lỗi đếm sót phát hiện trong lúc làm** (`tutor_service.py`, nhánh TNDS pha chốt):
    `except ValueError` trước đây gán `ket_qua` nhưng **quên `ket_qua_dict`** → lượt CAS-hỏng
    của TNDS lưu `turns.ket_qua_so_khop = NULL`. Mọi thống kê đếm theo cột này (kể cả cờ mới)
    **sót sạch ca TNDS**. Đã có test regression riêng, và đã **kiểm chứng test thật sự đỏ khi
    hoàn nguyên bản vá** (không phải test giả xanh).
  - `models/flag.py`: thêm `LoaiCo.khong_phan_tich_duoc`. Migration `c3f1a72d5b48`.
  - **⚠️ Migration enum — bài học quan trọng, khác mọi migration trước:** `flags.loai_co` là
    `sa.Enum(name='loaico')`. Trên SQLite (dev/test) chỉ là `VARCHAR(22)` không CHECK → no-op;
    trên **PostgreSQL (production) là native ENUM type**, bắt buộc `ALTER TYPE ... ADD VALUE`,
    **Alembic autogenerate KHÔNG sinh ra lệnh này** (phải viết tay, rẽ nhánh theo dialect).
    - **Phải bọc `op.get_context().autocommit_block()`**: Alembic mặc định bọc migration trong
      BEGIN/COMMIT, mà **PostgreSQL < 12 CẤM `ALTER TYPE ... ADD VALUE` trong transaction
      block** → lỗi cứng → **app không khởi động được** (vì tự `alembic upgrade head` lúc
      start). Bản đầu tôi viết thiếu chỗ này, phát hiện nhờ render `alembic upgrade --sql` với
      URL postgres (chế độ offline, không cần kết nối) thấy rõ `BEGIN; ALTER TYPE...; COMMIT;`.
      **Mẹo kiểm tra này nên dùng lại cho mọi migration chạm Postgres mà local chỉ có SQLite.**
    - `downgrade` **cố ý để trống** (PostgreSQL không có lệnh xóa giá trị khỏi enum) — ngoại lệ
      có chủ đích với quy ước "test upgrade/downgrade" ở CLAUDE.md mục 6, đã ghi rõ trong file.
    - Đã test `upgrade head → downgrade -1 → upgrade head` trên **bản sao dev.db thật**
      (254 turns / 15 flags / 21 sessions nguyên vẹn).
  - `admin_service.py`: thêm `nguong_co_khong_phan_tich: 3` (chỉnh được ở trang Cấu hình admin).
  - `tutor_service.py`: `_tu_dong_gan_co_khong_phan_tich()` — cùng khuôn `_tu_dong_gan_co_chot_chan`
    (flush → đếm → so ngưỡng → idempotent 1 cờ/phiên). **Đếm ở Python chứ không lọc SQL**:
    `ket_qua_so_khop` là cột JSON, cú pháp truy vấn JSON khác nhau giữa SQLite và PostgreSQL.
  - Frontend: nhãn "CAS không đọc được" ở **cả 2 nơi** (`gv/QuanLyCo.jsx` + `admin/NhatKy.jsx` —
    chỗ thứ hai rất dễ sót, thiếu thì admin thấy chuỗi thô); xếp vào nhóm **cờ NỘI DUNG** cùng
    `chot_chan_nhieu` nên chỉ có nút "Đã xử lý", KHÔNG có "Xử lý & nhắn HS" (nhắn HS "em gặp
    khó" là sai địa chỉ khi lỗi nằm ở dữ liệu bài); thêm ô chỉnh ngưỡng ở `admin/CauHinh.jsx`.
  - Build-test-fix: `ruff` sạch · `pytest` **653/653** (648 → +5: đủ ngưỡng gắn cờ · dưới ngưỡng
    không gắn · idempotent · đối chứng "sai nhưng CAS đọc được thì KHÔNG gắn" · regression TNDS)
    · `eslint` sạch · `vitest` 38/38 · `vite build` OK.
  - **Kiểm chứng trên app thật (dev.db)**: gửi 3 đáp án CAS không đọc được → cờ bắn đúng, GV
    thấy qua API và trên trang "Cờ theo dõi" với nhãn tiếng Việt, đúng bộ nút của cờ nội dung.

## 1c. Trạng thái trước đó (v163)

- **✨ (v163) ui: "Bài nên luyện tiếp" ở màn hình hoàn thành bài — khép vòng học tập.** User
  tự nhận định (không phải yêu cầu code): đề xuất bài kế nằm ở trang Chọn bài, không hiện
  ngay màn hình kết thúc bài → vòng học tập nhìn "đứt" ở khúc cuối, đặc biệt khi demo.
  - Tái dùng 100% dữ liệu/cơ chế đã có: `pt.diem_yeu` (từ `api.getPhanTichMe()`, đúng nguồn
    thẻ "Gợi ý cho em" ở Chọn bài) + `luyenDang()` (cơ chế điều hướng có sẵn của trang Tiến
    bộ). Không đụng backend, không endpoint mới, không migration.
  - `PhongHoc.jsx`: tải `phan-tich-me` CHỈ khi `trangThai.da_xong` chuyển `true` (không tải
    lúc vào bài, tránh thêm request vào đường nóng nhất; số liệu lúc này đã tính cả bài vừa
    xong). Lỗi fetch bị nuốt lặng lẽ (`.catch(() => {})`) — gợi ý là phần phụ, không được làm
    vỡ màn hình hoàn thành.
  - Thêm `frontend/src/utils/goiYTiepTheo.js::baiNenLuyenTiep(pt)` — hàm thuần, trả dòng
    `diem_yeu[0]` nếu có VÀ có `dang_id` hợp lệ, ngược lại trả `null` để `BangHoanThanh` ẩn
    hẳn dòng gợi ý (không hiện khung rỗng cho HS mới/chưa đủ dữ liệu — `NGUONG_NHOM=2`).
  - `HocSinhApp.jsx`: thêm `luyenDangCoCanhBao()` — bọc `luyenDang()` qua
    `canhBaoNeuCanRoiTrang()` cho nhất quán với `onChonBai`/`onTrangChu` sẵn có của
    `PhongHoc`. Đã KIỂM CHỨNG THỰC TẾ (không suy đoán) rằng cảnh báo "rời trang đang làm dở"
    không bao giờ bật ở đây: `PhongHoc` tự báo `onDangDo(false)` ngay khi `da_xong` — rủi ro
    duy nhất nêu trong kế hoạch coi như đã loại trừ bằng cách đọc code, không phải đoán.
  - Build-test-fix: `ruff`/`eslint` sạch · `pytest` 648/648 (không đổi backend) · `vitest`
    **38/38** (+4 test mới cho `baiNenLuyenTiep`: null/rỗng/thiếu `dang_id`/đủ dữ liệu) ·
    `vite build` OK · **e2e Playwright 4/4** (thêm ca "HS mới — dòng gợi ý ẩn hẳn, màn hình
    hoàn thành vẫn nguyên vẹn", khóa đúng hành vi thật của DB e2e mới seed — không giả vờ có
    dữ liệu để test cho đẹp).
  - **Verify nhánh CHÍNH (có gợi ý) trên dev.db thật**: không HS nào trong dev.db đủ dữ liệu
    yếu để tự nhiên trigger `diem_yeu`, nên dùng Playwright `page.route()` giả lập response
    `phan-tich-me` rồi mở thẳng 1 phiên đã hoàn thành — xác nhận dòng "Bài nên luyện tiếp:
    Tìm cực trị 42%" hiện đúng, bấm vào chuyển sang `#chon_bai` và áp đúng cơ chế lọc có sẵn.

## 1d. Trạng thái trước đó (v162)

- **✨ (v162) feat: GV/Admin xem lại được hội thoại của HS — kể cả bài đang làm dở.** User hỏi
  "GV muốn xem lại bài HS đã làm" — rà lại thì backend `/sessions/{id}/xem-lai` đã mở sẵn cho
  GV/Admin từ B3 nhưng frontend chưa có cửa vào nào; và endpoint chặn cứng phiên chưa hoàn
  thành, đúng lúc cờ "không hiểu nhiều" bắn ra là lúc phiên còn dang dở — chặn ở đó khiến GV
  xử lý cờ trong tình trạng mù, chưa từng thấy hội thoại đã xảy ra.
  - **Backend** (`sessions.py::xem_lai_phien`): điều kiện `phải hoàn thành` giờ CHỈ áp cho HS;
    GV/Admin xem được cả phiên `dang_lam`. Thêm vào payload (CHỈ cho GV/Admin, HS không thấy):
    `hoc_sinh_ten`, `trang_thai`, và 3 trường theo từng lượt `buoc`/`y`/`co_bi_chot_chan`.
    `loi_giai_chi_tiet` GV/Admin luôn thấy (trước đây phụ thuộc `hien_loi_giai_chi_tiet` — cấu
    hình đó chỉ có ý nghĩa với HS, GV là chủ nội dung không có lý do gì bị ẩn). Thêm
    `tong_buoc`/`mo_ta_cac_buoc` (cả 2 vai trò) để FE dựng dải phân cách bước.
  - **Backend** (`monitor.py::nhat_ky_hoan_thanh`): thêm tham số `hoc_sinh_id` (lọc 1 HS, kiểm
    `hoc_sinh_thuoc_gv` → 403 nếu ngoài phạm vi) + `trang`/`moi_trang` (phân trang server —
    trước đây cắt cứng 100 dòng gần nhất, không tra được bài của 1 HS cụ thể quá 100 dòng).
    Đổi shape trả về `{"rows": [...], "tong": N}` (BREAKING — sửa kèm cả 2 nơi gọi FE trong
    cùng commit, đã grep xác nhận chỉ 1 chỗ gọi trước khi đổi).
  - **Frontend** (`XemLaiBai.jsx`): thêm prop `vaiTro` ('hs' mặc định | 'gv') — mọi nhãn lệch
    vai (xưng "em" ↔ giọng GV) gom vào 1 bảng `NHAN[vaiTro]`, không rải if/else trong JSX. Dùng
    lại `dungDongChat()` (đã có từ v159) để hiện dải phân cách bước trong lịch sử; hiện badge
    "Đang làm dở" khi `trang_thai !== 'hoan_thanh'`; hiện nhãn "lượt này đã bị chốt chặn rò rỉ
    đáp án" ở turn có `co_bi_chot_chan` (chỉ GV thấy).
  - **3 cửa vào** cho GV, xếp theo giá trị nghiệp vụ: (a) **trang Cờ** — nút "Xem hội thoại" ở
    mỗi dòng có `session_id`, giá trị cao nhất vì trước đây GV xử lý cờ hoàn toàn mù; (b) **Nhật
    ký hoàn thành** — nút "Xem lại" cạnh "Gắn cờ", đổi phân trang sang server-side; (c) card
    **"Bài đã làm"** mới trong panel "Tiến độ chi tiết: {tên HS}" — đúng luồng "xem lại bài của
    em X" mà user hỏi ban đầu, **phân trang server-side 5 bài/trang** (không chỉ 10 bài gần
    nhất không phân trang như bản đầu — user yêu cầu sửa ngay sau khi xem qua), tái dùng
    `PhanTrang` + state `trangBaiHsChon`/`baiCuaHsChonTong`, tự về trang 1 khi đổi HS đang xem.
  - **Không đụng bất biến #3** (không lộ đáp án): GV vốn xem đáp án ở Quản lý câu hỏi, quy tắc
    đó chỉ ràng buộc lúc HS đang học. Rủi ro thật là #6 (phân quyền) — chốt bằng
    `hoc_sinh_thuoc_gv` (đã có sẵn, có test IDOR) + test khẳng định PHỦ ĐỊNH: payload trả cho
    HS không chứa các trường chỉ dành GV (`test_xem_lai_hs_khong_thay_truong_danh_cho_gv`).
  - Build-test-fix: `ruff` sạch · `pytest` **648/648** (642 → +6 test mới: GV xem được phiên
    dang_lam · HS vẫn 403 trên phiên dở của chính mình · payload HS không rò trường GV · GV
    luôn thấy lời giải chi tiết · lọc nhật ký theo `hoc_sinh_id` · 403 khi `hoc_sinh_id` ngoài
    phạm vi GV) · `eslint` sạch · `vitest` **34/34** · `vite build` OK · **e2e Playwright 4/4**
    (thêm 1 luồng vàng: HS làm dở 1 phiên → GV tự gắn cờ qua API → vào "Cờ theo dõi" → bấm "Xem
    hội thoại" → xác nhận overlay đổi giọng GV, hiện badge "Đang làm dở", hiện đúng lượt HS vừa
    gửi — không chỉ mở overlay trống).

## 1e. Trạng thái trước đó (v161)

- **🐞 (v161) fix: gợi ý mục tiêu vẫn hiện lại y hệt sau khi đã thêm (bug thật, user phát
  hiện ngay lúc dùng thử v160).** Ở v160 tôi khẳng định "đóng/mở lại gợi ý thì cái đã thêm
  tự nhiên biến mất" — **chưa hề kiểm chứng, và sai**. Tra `muc_tieu_service.de_xuat()` xác
  nhận hàm này chỉ tính từ điểm yếu (`ho_so_nang_luc`) + 2 gợi ý cố định (tuần/ngày), **không
  hề đọc bảng `muc_tieu`** — nên gợi ý luôn giống hệt nhau mỗi lần mở, bất kể HS đã đặt gì.
  - Thêm `_dang_co_chua_dat()`: quét mục tiêu ĐANG HOẠT ĐỘNG (`da_huy=False`) VÀ CHƯA ĐẠT của
    HS thành tập khóa `(loai, dang_id)`. `de_xuat()` lọc bỏ mọi gợi ý trùng khóa với tập này.
  - **Mục tiêu ĐÃ ĐẠT không bị chặn** — đúng góp ý gốc "khi nào hoàn thành thì mục tiêu đó mới
    sáng lên để chọn tiếp": tính `_tien_do()` so với `chi_tieu_so`, đạt rồi thì gợi ý cùng loại
    lại xuất hiện bình thường ở lần mở kế tiếp.
  - Mục tiêu đã XÓA (`da_huy=True`) cũng không tính là "đang có" — gợi ý xuất hiện lại ngay.
  - Build-test-fix: `ruff` sạch · `pytest` **642/642** (638 → +4 test mới: không lặp lại khi
    chưa đạt, sáng lại khi đã đạt, bỏ qua mục tiêu đã xóa, lọc đúng cả gợi ý theo điểm yếu
    `chu_de` chứ không chỉ 2 gợi ý cố định) · frontend không đổi lần này.
  - **Kiểm tra trên app thật đúng kịch bản user báo lỗi** (Playwright): thêm 1 gợi ý "tuần" →
    đóng gợi ý → mở lại → xác nhận dòng đã thêm **không còn**, chỉ dòng "ngày" (chưa thêm) vẫn
    hiện — khớp ảnh chụp user gửi kèm báo lỗi.

## 1f. Trạng thái trước đó (v160)

- **✨ (v160) ui: sửa hành vi nút "Gợi ý" trong trang Mục tiêu + thêm gợi ý "3 bài/ngày".**
  Trước đây nút "Hủy" ở MỖI hàng gợi ý đều gọi `setDeXuat(null)` → đóng CẢ danh sách; bấm
  "+ Thêm" cũng đóng cả danh sách sau khi thêm — không thêm được nhiều gợi ý liên tiếp.
  - **1 nút "Hủy" chung**: gộp vào nút toggle ở header (`Gợi ý` ⇄ `Đóng gợi ý`), giống hệt
    cơ chế nút "Đặt mục tiêu/Đóng" đã có sẵn — bỏ nút Hủy riêng từng hàng.
  - **Mỗi hàng 1 nút "Thêm" độc lập**: bấm hàng nào chỉ tạo mục tiêu hàng đó, đánh dấu qua
    state cục bộ `daThemGoiY` (Set index, reset mỗi lần mở lại danh sách) → hàng đó mờ +
    nút đổi "Đã thêm" (disabled), các hàng khác vẫn bấm được bình thường, danh sách **không**
    đóng lại.
    - ⚠️ **Lúc viết bản này tôi khẳng định "lần mở gợi ý kế tiếp, gợi ý đã thêm & chưa hoàn
      thành tự nhiên không xuất hiện nữa" — SAI, chưa kiểm chứng thật.** User dùng thử phát
      hiện ngay: đóng rồi mở lại gợi ý vẫn hiện y hệt dòng đã thêm. Tra lại `de_xuat()` thì
      xác nhận hàm này **CHƯA BAO GIỜ đọc bảng `muc_tieu`** — chỉ tính từ điểm yếu + 2 gợi ý
      cố định, hoàn toàn không biết HS đã đặt gì. Sửa thật ở v161 bên dưới.
  - **Thêm gợi ý mới "Hoàn thành 3 bài trong ngày hôm nay"**: thêm `loai='ngay'` — TÁI DÙNG gần
    như nguyên vẹn logic tính tiến độ của `loai='tuan'` (chỉ khác cửa sổ 1 ngày thay vì 7 ngày,
    xem `_tien_do()` trong `muc_tieu_service.py`).
  - **3 gợi ý khác đã phân tích nhưng KHÔNG làm đợt này** (đã trao đổi, user chốt phạm vi):
    - "Hoàn thành 60 phút liên tục" — **bỏ hẳn**, không dùng mục tiêu này nữa.
    - "5 ngày liên tiếp" (streak) và "1 đề thi thử trong tuần" — **hoãn**, mỗi loại cần cơ chế
      đo backend riêng (streak cần nhánh tính khác `tinh_chuoi_ngay()` hiện có vì phải tính từ
      mốc đặt mục tiêu chứ không phải tới hôm nay; đề thi thử cần đọc từ bảng `bai_thi` — nguồn
      dữ liệu khác hẳn `sessions` mà toàn bộ hệ mục tiêu hiện chỉ đọc từ đây).
  - Build-test-fix: `ruff` sạch · `pytest` **638/638** (636 → +2 test mới: `test_tien_do_ngay`,
    `test_tien_do_ngay_cua_so_hep_hon_tuan` — ca sau xác nhận cửa sổ 'ngay' HẸP HƠN 'tuan' bằng
    cách đẩy lùi `moc_bat_dau` 2 ngày rồi kiểm tra bài hoàn thành hôm nay lọt cửa sổ tuần nhưng
    KHÔNG lọt cửa sổ ngày) · `eslint` sạch · `vitest` **34/34** · `vite build` OK.
  - **Kiểm tra trên app thật cả HS lẫn GV** (Playwright, component `MucTieuPanel` dùng chung):
    xác nhận nút header đổi `Gợi ý` ⇄ `Đóng gợi ý`; bấm "+ Thêm" 1 dòng → dòng đó mờ + "Đã
    thêm", dòng còn lại vẫn sáng, danh sách vẫn mở; mục tiêu mới xuất hiện ngay trong danh sách
    bên dưới; panel phía GV (mở qua "Xem tiến độ" 1 HS) hoạt động giống hệt.
...
- **✨ (v158) ui: thêm thẻ "Gợi ý cho em" ở trang Chọn bài (HS).** User yêu cầu thêm 1 thẻ
  ...
  dung cụ thể (thẻ gốc gồm 3 phần: cảnh báo thiếu dữ liệu + Điểm mạnh/Cần cải thiện + Đề xuất)
  — user chọn **"Điểm yếu + nút Luyện ngay"**.
  - `ChonBai.jsx`: gọi thêm `api.getPhanTichMe()` trong `Promise.allSettled` hiện có (lỗi phụ
    không chặn danh sách bài, đúng pattern sẵn có) → `pt`. Card chỉ hiện khi
    `pt.diem_yeu.length > 0` (không có điểm yếu → không chiếm chỗ).
  - Mỗi dòng: tên dạng + % thành thạo (đỏ, cùng style `PhanTichNangLuc`), nút "Luyện ngay" nếu
    có `dang_id`. Tái cấu trúc logic `locBanDau` (vốn chỉ áp bộ lọc lúc mount từ điều hướng
    trang khác) thành hàm dùng chung `apDungLoc({chuyen_de?, dang_id?, trang_thai?})` — nút
    "Luyện ngay" gọi lại hàm này để set filter **ngay tại chỗ** (khác luồng cũ ở Tiến độ phải
    điều hướng sang trang khác).
  - **Kiểm tra bằng app thật** (bắt buộc quy tắc UI, không chỉ dựa lint/test): bật backend +
    frontend dev, đăng nhập HS thật, dùng Playwright điều khiển trình duyệt thật. Không tài
    khoản seed nào có sẵn dữ liệu "điểm yếu" thật → mock riêng response
    `/progress/me/phan-tich` (chỉ để có dữ liệu test), mọi thứ khác (đăng nhập, trang,
    component, nút bấm) chạy thật. Xác nhận: card ẩn đúng khi rỗng, hiện đúng khi có dữ liệu,
    bấm "Luyện ngay" set đúng Chuyên đề + Dạng và danh sách bài lọc lại chính xác (có screenshot
    đối chiếu).
    - Bắt 1 lỗi TRONG LÚC viết script test (không phải bug thật): mock ban đầu gõ sai hoa/thường
      "Đạo hàm" trong khi dữ liệu thật là "đạo hàm" → tưởng nhầm là bug khớp tên chuyên đề, sau
      xác minh lại là lỗi chính tả trong mock, không phải lỗi code.
  - Đổi mật khẩu tạm 5 tài khoản HS demo trên `dev.db` LOCAL (không phải production, không
    commit) để có tài khoản đăng nhập test — đã báo user, có thể re-seed nếu cần lại mật khẩu
    cũ.
  - Build-test-fix: `eslint` sạch · `vitest` 23/23 (không đổi backend nên không chạy lại
    `pytest`) · `vite build` OK.

## 1h. Trạng thái trước đó (v157)

- **✨ (v157) ui: chuẩn hóa 5 chỗ chữ trong giao diện (không đụng logic).** User yêu cầu thay
  trực tiếp, không cần phân tích trước.
  - **Toàn bộ "—" → "-" trong text UI** (179 chỗ, 50 file `frontend/src`): dùng script quét có
    nhận biết state chuỗi/comment (không phải regex mù) — chỉ đổi ký tự nằm trong text HIỂN
    THỊ THẬT (JSX text, `subtitle=`/`label=`/`placeholder=`, ô "—" thay dữ liệu trống...), GIỮ
    NGUYÊN mọi code comment (207 dòng "—" trong comment không đổi) và không đụng en-dash "–"
    (vd "0–100", "A–D" — khoảng số/chữ, khác ký tự). Đã hỏi phạm vi riêng: user chọn CHỈ
    frontend, không đụng 442 chỗ "—" trong backend (chủ yếu docstring/comment nội bộ).
    - **Bug tự phát hiện giữa chừng**: bản script đầu chỉ bắt "—" nằm NGOÀI chuỗi (bỏ sót mọi
      `subtitle="...—..."` — phần lớn text UI thật lại nằm trong chuỗi!) → dry-run đối chiếu
      thủ công phát hiện thiếu (vd `AISinhCauHoi.jsx:494` không được bắt dù có "—") → viết lại
      state machine cho đúng (chuỗi CŨNG là "code" cần thay, chỉ // và /* */ mới là comment
      cần bỏ qua) → quét lại: 59 chỗ (bản lỗi) → 162 dòng / 179 ký tự (bản đúng).
    - Kéo theo sửa đồng bộ 2 test khớp giá trị nguồn đã đổi: `utils/format.test.js`
      (`dinhDangThoiGian(null)` nay trả `'-'`) và `utils/cauHoi.test.js` (label
      `'- Chưa gán dạng -'`) — nếu không sửa test sẽ đỏ vì nguồn (`format.js`/`cauHoi.js`) đã
      đổi giá trị thật.
  - **4 cụm từ đổi đúng nghĩa** (tìm bằng chuỗi chính xác, chỉ đổi khi khớp UI thật, KHÔNG đổi
    code comment gần giống): "ít cần gợi ý hơn = đang tiến bộ" → "..., nghĩa là đang tiến bộ"
    (`The7NgayQua.jsx`); "Thời gian TB mỗi bài" → "Thời gian trung bình mỗi bài"
    (`The7NgayQua.jsx`); "Lượt cần gợi ý TB mỗi bài" → "...trung bình mỗi bài"
    (`The7NgayQua.jsx`); "AI tự bịa đề" → "AI tự sinh đề" (`AISinhCauHoi.jsx` subtitle — có 1
    chỗ khác là code comment giải thích hành vi nội bộ, giữ nguyên vì không phải giao diện).
...
- **🧹 (v156) chore: dọn code chết "Yêu cầu đặt lại" + đưa `migrate.py` vào cổng CI.** Rà soát
  ...
  - **A — Bộ 3 file chết "Yêu cầu đặt lại"** (sót lại từ commit `3e34cc3` khi đổi thiết kế
    "GV xin → Admin duyệt" sang "GV đặt lại tiến độ HS TRỰC TIẾP"): đã xóa
    - `backend/app/models/yeu_cau_dat_lai.py` — model `YeuCauDatLai` không import ở đâu trong
      `app/`, không nằm trong `models/__init__.py` lẫn Alembic baseline (bảng chưa bao giờ được
      tạo trên DB mới).
    - `frontend/src/pages/admin/YeuCauDatLai.jsx` — trang KHÔNG được route (`QuanTriApp.jsx`
      chỉ lazy-load 8 trang khác); còn gọi 3 method `api.adminDanhSachYeuCauDatLai` /
      `adminDuyetDatLai` / `adminTuChoiDatLai` KHÔNG tồn tại trong `api.js` → sẽ crash nếu lỡ
      route lại. Luồng thật đang dùng: `gvDatLaiTienDo` → `POST /gv/dat-lai/{hs_id}`
      (`dat_lai_service.py`).
    - `migrate.py` — gỡ `yeu_cau_dat_lai` khỏi `TABLES`, khỏi block import model, khỏi bước
      insert #15 (bảng cũ không còn trong schema).
  - **B — `migrate.py` vào CI**: trước đây cổng CI chỉ chạy `ruff check app/` → 20 lỗi ruff
    trong `migrate.py` (script di trú SQLite→Postgres ở gốc `backend/`) là VÙNG MÙ, không bao
    giờ bị bắt. Đã sửa sạch (`io` thừa, 12 dòng `_log(); total +=` tách khỏi `;` gộp câu) và
    đổi CẢ HAI nơi kiểm tra sang `ruff check app/ migrate.py`: `.github/workflows/ci.yml` +
    `scripts/kiem-tra-truoc-khi-push.ps1` (giữ đồng bộ theo CLAUDE.md §2a). KHÔNG mở rộng sang
    `ruff check .` vì sẽ kéo theo 4 lỗi I001 ở `alembic/versions/*` (file tự sinh) — ngoài
    phạm vi.
  - Build-test-fix: `ruff check app/ migrate.py` sạch · import smoke OK · `pytest` (chờ xác
    nhận cuối) · `eslint`/`vitest` (23/23) · `vite build` OK. Không đổi hành vi runtime — chỉ
    xóa code chết + siết cổng lint.

## 1j. Trạng thái trước đó (v155)
...
- **✨ (v155) ui: rà soát toàn bộ codebase, loại bỏ emoji trang trí/chức năng, thay bằng
  ...
  backend (text do BE sinh: thông báo, gợi ý mặc định, cột mốc streak).
  - **Quy tắc mới** (`frontend/src/styles/README.md`): KHÔNG dùng emoji ở bất kỳ đâu trong UI,
    kể cả câu chào/động viên thuần văn bản (trước đây cho phép — nay bỏ hẳn theo lựa chọn của
    user khi được hỏi). Icon chức năng dùng `lucide-react`, tái dùng icon đã có trong codebase
    (`Check`/`X`/`AlertTriangle`/`Target`/`Lightbulb`/`Sparkles`/`Flag`/`Printer`... — xem bảng
    ánh xạ đầy đủ trong code).
  - **Ngoại lệ giữ nguyên có chủ đích** (không phải sót):
    - `tutor_service.py`/`tro_giup_service.py` (BE): tiêu đề thông báo GV `🆘 KHẨN CẤP` (nghi
      khủng hoảng/tự hại) vs `⚠️` (nội dung cần chú ý thường) — hỏi lại user riêng, user chọn
      GIỮ NGUYÊN vì đây là tín hiệu an toàn HS giúp GV phân biệt mức độ khẩn cấp ngay trên tiêu
      đề thông báo (chuông thông báo chỉ render text thuần, không có icon), không phải trang
      trí — không đụng vào an toàn HS như một tác dụng phụ của việc dọn thẩm mỹ.
    - `MathPalette.jsx`: `⌫` (backspace) — ký hiệu kỹ thuật cạnh √/π/∫, không phải emoji trang
      trí.
    - `CauHinh.jsx` field `icon: '✨'` (giá trị mặc định khi Admin thêm bước hướng dẫn mới) và
      `data/seed/problems.json` (`✓` trong `loi_giai_y`, nội dung lời giải nội bộ KHÔNG hiển
      thị cho HS) — dữ liệu tự do người dùng nhập / nội dung nội bộ, không phải UI chrome cố
      định.
    - `GiaoVienApp.jsx`/`QuanLyCo.jsx`: code comment mô tả đúng hành vi BE ở trên, không đổi.
  - **Đổi HỢP ĐỒNG dữ liệu BE↔FE** cho `huong_dan_phong_hoc` (hướng dẫn 3 bước ở Phòng học):
    `admin_service.py` gửi `icon` là KHÓA TÊN (`"compass"`/`"lightbulb"`/`"pen"`) thay vì emoji
    thô; `HuongDanPhongHoc.jsx`'s `renderIcon()` map khóa biết trước sang icon `lucide-react`,
    khóa lạ (Admin tự gõ ký tự khác ở trang Cấu hình) vẫn hiển thị nguyên ký tự — không giới
    hạn ô nhập tự do của Admin.
  - Cột mốc streak (`chuoi_ngay_service.py`, 8 tiêu đề `🎯⭐🌟💪🏆🔥🏅`) và văn bản
    `StubLLMClient`/`phan_tich_service.py` (emoji động viên/xu hướng) → bỏ hẳn, không thay thế
    (là text thuần render trong chuông thông báo, không có chỗ gắn icon).
  - Build-test-fix: `ruff check app/` sạch (lỗi ở `migrate.py` gốc, có từ trước, không liên
    quan phiên này) · import smoke OK · `pytest` **627/627** (xác nhận 2 test
...
- **🔒 (v154) security: thu hồi token qua `token_version` — token cũ HẾT hiệu lực ngay khi đổi
  ...
  chỉ mang `sub`+`exp` (`jwt_expire_minutes=480` = 8h), đổi mật khẩu KHÔNG đá được token đã lộ
  — nó vẫn sống tới 8h.
  - **Cơ chế**: thêm cột `users.token_version` (Integer, default 0). JWT mang theo `tv` =
    token_version lúc phát. `auth/deps.py` mỗi request so `payload.get("tv", 0)` với
    `user.token_version` (user vốn ĐÃ được `db.get` mỗi request để kiểm khóa → phép so gần như
    miễn phí, giữ nguyên ưu điểm stateless, KHÔNG cần bảng session/Redis). Lệch = 401.
  - **Helper `vo_hieu_hoa_token_cu(user)`** (`auth/security.py`) tăng version; gọi ở 7 chỗ: 5
    chỗ đổi mật khẩu (HS/GV/Admin tự đổi + GV đổi HS + Admin đổi GV/HS) + 2 chỗ khóa tài khoản
    (khóa = công tắc thu hồi thật, token cũ chết ngay VÀ vẫn chết cả sau khi mở khóa lại, tránh
    token bị lộ "sống lại").
  - **Tương thích ngược**: token đời trước (không có claim `tv`) → `.get` mặc định 0, khớp
    `token_version=0` của tài khoản chưa từng đổi mật khẩu → KHÔNG đá văng ai lúc deploy; chỉ
    hết hiệu lực khi thật sự đổi mật khẩu/khóa sau này.
  - **Migration** `fa758f2e3937`: `server_default='0'` (autogenerate BỎ SÓT — thêm cột NOT NULL
    vào bảng users đã có dữ liệu production mà không có default sẽ lỗi; đã sửa tay). Đã test
    upgrade→downgrade→upgrade round-trip trên BẢN SAO dev.db, mọi dòng cũ nhận token_version=0.
  - **Frontend KHÔNG đổi**: `api.js` đã tự đẩy mọi 401 về màn đăng nhập. Lưu ý UX: đổi mật khẩu
    CỦA CHÍNH MÌNH → thao tác kế tiếp bị đẩy về đăng nhập lại (chuẩn, an toàn); GV/Admin đổi mật
    khẩu NGƯỜI KHÁC thì phiên của họ không bị ảnh hưởng.
  - Test: 3 test mới (`test_auth.py`: đổi mật khẩu vô hiệu token cũ; token không-`tv` vẫn chạy;
    khóa vô hiệu cả sau khi mở lại) + sửa `test_hs_sua_ho_ten_va_mat_khau` (cũ reuse token cũ
    sau đổi mật khẩu → nay đúng ra 401; assert thẳng trên response PATCH + login lại). `ruff`
    sạch · `pytest` **627/627** · frontend không đổi.
  - **Còn lại từ rà soát ngoài** (chưa làm, có lộ trình — xem đánh giá cho thuyết trình): #2
...
- **✨ (v153) ui: cảnh báo rời trang khi HS đang làm dở (phòng học/thi thử) + mỗi câu 1 dòng ở
  ...
  làm sau" hoặc chuyển trang khác → hỏi xác nhận; (2) đang thi mà chưa hết giờ/chưa nộp, bấm
  chuyển trang khác → hỏi xác nhận; (3) văn bản AI ở "Đánh giá tổng quan" (cả màn HS và GV)
  đang dồn thành 1 đoạn dài, cần mỗi câu xuống 1 dòng.
  - `PhongHoc.jsx`: thêm prop `onDangDo`, effect báo lên `!loading && !error && !!problem &&
    !trangThai.da_xong` (chỉ tính "đang dở" khi bài đã tải xong, không tính lúc loading/lỗi).
  - `ThiThu.jsx`: thêm prop `onDangLamBai`, effect báo `man.ten === 'thi'` (đang ở màn làm bài,
    chưa chuyển sang `ket_qua`).
  - `HocSinhApp.jsx`: hàm `canhBaoNeuCanRoiTrang()` (dùng `useConfirm` sẵn có, theo đúng mẫu
    `moPhongHocTuThongBao` đã có) — hỏi xác nhận đúng ngữ cảnh (2 thông điệp khác nhau cho
    phòng học/thi thử). Bọc TẤT CẢ đường điều hướng chủ động của HS (menu/tab điều hướng qua
    `onNavigate`, nút trong `PhongHoc`, bấm vào thông báo ở chuông) bằng
    `dieuHuongCoCanhBao`/kiểm tra tương ứng — không chỉ riêng nút "Quay lại làm sau".
    - Không bọc: back/forward trình duyệt và đóng tab/refresh (`beforeunload`) — cơ chế khác
      hẳn (không dùng được `useConfirm`, dễ vỡ UX với hash-routing), ngoài phạm vi yêu cầu.
    - Gặp 1 lỗi eslint `react-hooks/purity` khi gọi `Date.now()` bên trong hàm `async` —
      chuyển `moTuThongBao` về hàm thường + `.then()` thay vì `async/await` để né heuristic
      của rule (không phải bug thật, chỉ là rule quá thận trọng với `async function` lồng
      trong component).
  - `PhanTichNangLuc.jsx`: thêm `tachCauVanBan()` — tách theo dấu kết câu (`.`/`!`/`?`/`…`)
    theo sau là khoảng trắng (tránh tách nhầm số thập phân dạng "3.5"), render mỗi câu 1 `<p>`
    thay vì 1 khối `whitespace-pre-line` (vốn không tác dụng gì vì văn bản AI không có `\n`
    thật — xác nhận qua `SYSTEM_PHAN_TICH`/bản dự phòng theo luật, cả hai đều không chèn `\n`).
  - Test bằng mắt trên `dev.db` local (Playwright): xác nhận cả 2 dialog cảnh báo hiện đúng
    lúc, đúng nội dung, nút "Ở lại làm tiếp" giữ nguyên trang, "Rời khỏi" mới điều hướng; bấm
    "Tạo phân tích" thật cho `a1hs1` (dev.db đã có cấu hình Gemini thật, ghi đè `LLM_PROVIDER`
...
- **🐞 (v152) fix: "Giao bài ngay" (v151) chọn đúng HS nhưng KHÔNG mở sẵn đúng chuyên đề/dạng
  ...
  yếu" (nhỏ, phía trên) được khoá đúng dạng — accordion "Chọn câu hỏi" chính (nơi GV thực sự
  chọn bài) vẫn đóng, GV vẫn phải tự tìm lại.
  - **Nguyên nhân gốc**: quên nối `openCd`/`openDang` (state điều khiển accordion) theo
    `goiY`. Ngoài ra accordion nhóm theo tên dạng THUẦN (`b.dang_ten`), trong khi field duy
    nhất đang có (`r.ten`) là chuỗi "chuyên đề › dạng" ĐÃ GHÉP sẵn để hiển thị — không khớp
    key nhóm nên có nối cũng sai.
  - **Sửa**: `phan_tich_service.py` — thêm field `dang_ten` (tên THUẦN) vào `dang_meta`, tách
    biệt với `ten` (chuỗi ghép hiển thị). `GiaoVienApp.jsx` — `giaoBaiNgay` truyền thêm
    `chuyenDe`/`dangTenThuan`. `GiaoNhiemVu.jsx` — khởi tạo `openCd`/`openDang` bằng lazy
    initializer từ `goiY.chuyenDe`/`goiY.dangTenThuan` (đúng key `${cd}::${dang}` accordion
    đang dùng).
  - Test bằng mắt lại trên `dev.db` local (seed tạm 2 phiên yếu cho a1hs1 y hệt lần trước, đã
    xoá sau khi xác nhận): bấm "Giao bài ngay" → chuyên đề VÀ dạng cùng tự mở, thấy ngay các
    câu hỏi trong dạng đó kèm checkbox để chọn. `pytest` 624/624 · `eslint`/`vitest` 23/23 ·
    `vite build` sạch.

## 1n. Trạng thái trước đó (v151)
...
- **✨ (v151) ui: nút "Giao bài ngay" ở mục "Cần cải thiện" (Tiến bộ chi tiết học sinh, GV).**
  ...
  chuyên đề/dạng đang yếu để GV chọn bài — thay vì GV phải tự tìm lại từ đầu.
  - `PhanTichNangLuc.jsx`: thêm prop `onGiaoBai`, render nút "Giao bài ngay" cạnh mỗi dòng
    "Cần cải thiện" (chỉ khi được truyền — nên chỉ hiện phía GV, HS không có nút này vì
    `HocSinhApp` không truyền prop).
  - `GiaoVienApp.jsx`: state `goiYNv` + hàm `giaoBaiNgay(hs, r)` — điều hướng sang trang
    "Giao nhiệm vụ" kèm theo HS + dạng vừa bấm (theo đúng pattern hash-nav + focus state đã
    dùng cho `focusYc`/`focusCo`).
  - `GiaoNhiemVu.jsx`: nhận prop `goiY`/`onGoiYDone` — khởi tạo state (HS đã chọn, dạng khoá,
    tiêu đề gợi ý) bằng LAZY INITIALIZER từ `goiY` thay vì set trong effect (tránh lỗi eslint
    `react-hooks/set-state-in-effect` — cascading render); effect chỉ còn lo phần THẬT SỰ là
    side effect (gọi API gợi ý qua `gvDeXuatTheoDang` đã có sẵn — dạng-specific, khác API
    "quét toàn bộ điểm yếu" cũ — + báo `GiaoVienApp` đã tiêu thụ xong `goiY`). Thêm banner
    "Đến từ Giao bài ngay — đang khoá dạng «...»" kèm nút "Bỏ khoá" quay về chế độ gợi ý
    chung; đổi HS thủ công ở dropdown gợi ý cũng tự bỏ khoá.
  - Test bằng mắt trên `dev.db` local (KHÔNG phải production): tạo tạm 2 phiên chất lượng
    thấp cho `a1hs1` ở dạng "Tính đơn điệu của hàm số" (48%, "Cần luyện thêm"), bấm "Giao bài
    ngay" trên trình duyệt thật (Playwright script tạm) — xác nhận đúng HS được tick sẵn,
...
- **🔧 (v150) fix: khóa trần "manh" khi tỉ lệ hoàn thành quá thấp — phát hiện qua kiểm chứng
  ...
  mới), đã lấy `DATABASE_URL` production tạm thời (chỉ đọc, đã xin phép, không lưu lại) chạy
  trực tiếp `ho_so_nang_luc()` để kiểm chứng: đúng case gốc trong ảnh chụp ban đầu (HS
  "Học sinh 1-A1", dạng "Tính đơn điệu của hàm số", cạn gợi ý 5 lần/nhờ thầy cô 1 lần khớp
  chính xác dữ liệu thật) đã giảm từ **98% "Mạnh"** (công thức cũ, tính lại đúng bằng code cũ
  để đối chiếu) xuống còn **67% "Ổn"** — xác nhận sửa đúng vấn đề gốc.
  - **Phát hiện thêm** (cũng từ dữ liệu thật, dạng "Cực trị của hàm số" cùng HS): hoàn thành
    chỉ 2/7 bài (29%) nhưng 2 bài đó làm sạch (không cạn gợi ý) lại ra 77% **"Vững"** — vì
    công thức v149 giảm trọng số tỉ lệ hoàn thành (0.4→0.25) để nhường chỗ cho tín hiệu chất
    lượng, nên 1-2 bài làm tốt có thể che mất việc bỏ dở phần lớn bài đã bắt đầu. Câu `ly_do`
    vẫn nói đúng "Còn hay bỏ dở giữa chừng" nhưng nhãn màu xanh "Vững" dễ khiến người xem lướt
    qua tưởng ổn.
  - **Sửa**: `_phan_loai()` nhận thêm `ty_le_hoan_thanh`, thêm `NGUONG_TY_LE_KHOA_MANH = 0.6`
    (dùng CHUNG ngưỡng với câu `ly_do` để nhãn và lời giải thích luôn khớp nhau) — tỉ lệ hoàn
    thành dưới ngưỡng này thì khóa trần ở "kha" ("Ổn"), không cho lên "manh" ("Vững") dù điểm
    số tính ra ≥75. Không đụng ranh giới "kha"/"can_cai_thien".
  - Test mới `test_ty_le_thap_khong_duoc_xep_manh` (`test_progress.py`) — khóa đúng hành vi
    vừa phát hiện từ dữ liệu thật. `ruff` sạch · `pytest` 624/624 (không đổi frontend, không
    cần chạy lại eslint/vitest/build).

## 1p. Trạng thái trước đó (v149)
...
- **🔧 (v149) fix: cải tổ công thức "điểm thành thạo" (mạnh/yếu) — trước đây một dạng bài có
  ...
  chụp màn "Theo dạng bài", yêu cầu rà soát toàn dự án và đề xuất cải tiến trước khi code
  (đã duyệt qua artifact phân tích trước khi bắt tay code).
  - **Nguyên nhân gốc** (`phan_tich_service.py`): công thức cũ dùng `s.diem` thô — nhưng
    `diem` của TN4PA/TLN khi hoàn thành LUÔN = 1.0 (`tutor_service.py`, chỉ TNDS có bậc thang)
    nên gần như không phân biệt được ai làm trơn tru, ai loay hoay; đồng thời phạt gợi ý theo
    `cap_goi_y_hien_tai` — giá trị RESET về 0 mỗi khi trả lời đúng, nên chỉ phản ánh bậc gợi ý
    của BƯỚC CUỐI, không phải cả hành trình. Cột "cạn gợi ý"/"nhờ thầy cô" hiển thị trên UI
    hoàn toàn tách biệt khỏi công thức điểm → hai tín hiệu trái ngược nhau cùng hiện ra.
  - **Sửa (`_diem_thanh_thao` viết lại)**: đổi sang `diem_chat_luong_tb` (tái dùng tín hiệu
    của `_diem_xu_huong` — ưu tiên `diem_qua_trinh`, trừ dần theo số lần sai/không hiểu),
    giảm trọng số tỉ lệ hoàn thành (0.4→0.25), phạt theo `het_goi_y_tb` (trung bình
    `so_lan_het_goi_y` — KHÔNG reset trong phiên — của các phiên đã hoàn thành) thay cho
    `goi_y_tb` cũ. Xu hướng (`_xu_huong`) và điểm thành thạo giờ dùng CHUNG một tín hiệu chất
    lượng thay vì hai nguồn lệch nhau.
  - **Nhãn hiển thị hợp nhất**: thêm `NHAN_HIEN_THI` (Vững/Ổn/Cần luyện thêm/Chưa đủ dữ liệu)
    làm nguồn CHUẨN DUY NHẤT trong `phan_tich_service.py` — trả về qua `nhan_hien_thi` ở mỗi
    dòng. FE bỏ 2 bảng nhãn cục bộ từng lệch nhau (`PhanTichNangLuc.jsx` ghi "Mạnh",
    `BaoCaoPhuHuynh.jsx` ghi "Tốt" cho cùng nhãn `manh`).
  - **Câu giải thích ngắn**: thêm `ly_do` (vd "Còn cần nhiều gợi ý.", "Làm bài tốt, ít cần trợ
    giúp.") ở mỗi dòng — hiện ngay dưới thanh % ở `PhanTichNangLuc.jsx` để nhìn là hiểu vì sao
    ra điểm đó, không phải đoán.
  - **Màu sắc chỉ báo động ở mức cần hành động**: `frontend/src/utils/nangLuc.js` (mới) —
    nguồn TONE/màu dùng chung cho 2 màn. Đổi mức "Ổn" (kha) từ tone `warning` (vàng, dễ hiểu
    nhầm là có vấn đề) sang `primary` — chỉ mức "Cần luyện thêm" mới dùng `danger`.
  - Cập nhật `test_ban_do.py` (giá trị kỳ vọng tính lại đúng công thức mới, không tính tay) +
    `test_progress.py` (sửa docstring/assertion phản ánh so_lan_het_goi_y giờ ảnh hưởng thật
    tới điểm, thêm `test_het_goi_y_giam_diem_thanh_thao`). `ruff` sạch · `pytest` 623/623 ·
    `eslint`/`vitest` 23/23 · `vite build` sạch.

## 1q. Trạng thái trước đó (v148)
...
- **⚡ (v148) chore: tối ưu cổng chặn push (v146) — cache theo "commit sha + hash script
  ...
  hồi: push tag riêng sau khi vừa push nhánh main (cùng commit) vẫn chạy lại toàn bộ CI cục
  bộ — lãng phí. `.git/hooks/pre-push`: trước khi gọi `scripts/kiem-tra-truoc-khi-push.ps1`,
  đọc mọi ref đang push (từ stdin, đúng giao thức pre-push của git), so với cache
  `.git/kiem-tra-truoc-khi-push.da-xac-nhan` (mỗi dòng `<sha>:<hash script>`, hash tính qua
  `git hash-object` trên chính file .ps1 đang nằm trên đĩa). MỌI ref đã có trong cache →
  bỏ qua hẳn; CÓ ref mới → chạy đủ như cũ rồi ghi cache. Đổi script kiểm tra (thêm bước CI
  mới) tự làm hash đổi → cache cũ tự hết hiệu lực, không cần nhớ xóa tay.
  - Test: tự kiểm chứng bằng dry-run push thật (tag tạm trỏ HEAD, xóa sau khi xong, không
...
- **🐞 (v147) fix: "Nhận dạng đề từ ảnh" hiện công thức chữ thô (vd "overrightarrowAB" thay vì
  ...
  User báo qua ảnh chụp: ô "Ý a)"/"Ý b)"... chứa văn bản `$\\overrightarrow{AB}...$` (2 dấu \\
  liền nhau) và phần xem trước hiện chữ thô mất hết ký hiệu toán.
  - **Chẩn đoán** (dùng Explore agent điều tra kỹ trước khi sửa): `_parse_json_doc_de_tu_anh`
    (`backend/app/llm/client.py`) dùng CHUNG cơ chế vá JSON (`_va_escape_json`) với luồng sinh
    câu hỏi text — nhưng cơ chế vá đó CHỈ kích hoạt khi `json.loads` lần đầu THẤT BẠI (lỗi cú
    pháp). Nếu Gemini trả JSON escape THỪA (4 ký tự `\` thay vì 2 cho 1 lệnh LaTeX) — vẫn là
    JSON HỢP LỆ CÚ PHÁP — `json.loads` thành công ngay, không có lỗi nào để kích hoạt vá, nên
    lọt qua với nội dung sai (2 dấu `\` thật thay vì 1) → KaTeX hiểu "\\" là lệnh xuống dòng,
    công thức vỡ. Nguyên nhân gốc: `SYSTEM_DOC_DE_TU_ANH`/`user_prompt_doc_de_tu_anh` KHÔNG
    hề dặn AI cách escape backslash trong chuỗi JSON — khác `user_prompt_sinh_cau_hoi` đã có
    dòng dặn này từ trước (commit cũ, patch cho đúng lỗi này ở luồng khác) — luồng đọc ảnh
    (thêm sau) không được thừa hưởng bài học đó.
  - **Sửa #1 (root cause)**: thêm vào `SYSTEM_DOC_DE_TU_ANH` dòng dặn rõ + VÍ DỤ cụ thể "trong
    CHUỖI JSON, dấu \ LaTeX phải viết KÉP" (giống cách `user_prompt_sinh_cau_hoi` đã làm) —
    đã xác minh CHÍNH XÁC số lượng backslash sinh ra bằng script `repr()`, không đoán tay.
  - **Sửa #2 (lưới an toàn)**: thêm `_sua_backslash_thua_latex()` (`client.py`) — dò `\\\\lenh`
    (4 ký tự \, thừa 1 lớp) ngay trước 1 lệnh LaTeX quen thuộc (overrightarrow, sqrt, frac,
    vec, widehat...) và TỰ SỬA về `\\lenh` đúng chuẩn; áp dụng đệ quy cho `de_bai` VÀ
    `meta_nhap` (phương án/ý lồng nhau). An toàn vì domain này không dùng "\\" (xuống dòng
    LaTeX thật) ở bất kỳ đâu (cấm môi trường `aligned` từ v141) — 2 dấu `\` liền trước 1 lệnh
    LUÔN là escape thừa, không có case hợp lệ nào bị sửa nhầm (đã test: chuỗi đã đúng 1 dấu
    không bị đụng, văn bản thường không bị đụng).
  - Test: 6 test mới (`test_doc_de_tu_anh.py`) — sửa đúng backslash thừa ở `de_bai` + ở
    `meta_nhap` lồng nhau, không sửa nhầm chuỗi đã đúng/văn bản thường, khóa quy tắc prompt.
    Đã kiểm chứng bằng `repr()` thực tế cho cả prompt lẫn hàm sửa (không đoán tay số lượng
    backslash — bài học từ sự cố escape bash→JS trước đây trong session này). `ruff` sạch ·
    `pytest` 622/622.

## 1s. Trạng thái trước đó (v146)
...
- **✨ (v125) #5–#8 (P2, đợt rà soát 2026-07-18): nén PROGRESS.md, cập nhật docs, gắn Sentry,
  ...
  - **#5**: `docs/PROGRESS.md` 1757→228 dòng — lịch sử v113–v120 chuyển sang
    `docs/PROGRESS_ARCHIVE.md` mới; SỬA LUÔN lỗi trùng nhãn `## 1p.` (2 mục khác nhau cùng
