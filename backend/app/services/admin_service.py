...
def phien_theo_ngay(db: Session, so_ngay: int = 30) -> list[dict]:
    """Số phiên học BẮT ĐẦU mỗi ngày toàn hệ thống (kể cả ngày 0 phiên) — biểu đồ vùng
    "nhịp sử dụng" ở Dashboard, bổ sung chiều thời gian cho con số tĩnh `so_phien`."""
    hom_nay = datetime.now(timezone.utc).date()
    khung = [(hom_nay - timedelta(days=i)).isoformat() for i in range(so_ngay - 1, -1, -1)]
    gom = dict.fromkeys(khung, 0)
    tu_luc = datetime.now(timezone.utc) - timedelta(days=so_ngay)
    rows = (
        db.query(SessionModel.bat_dau_luc)
        .filter(SessionModel.bi_an == False, SessionModel.bat_dau_luc >= tu_luc)  # noqa: E712
        .all()
    )
    for (dt,) in rows:
