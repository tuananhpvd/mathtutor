...
ĐỐI CHIẾU BẮT BUỘC TRƯỚC KHI TRẢ KẾT QUẢ: "dap_an_dung"/"dap_an"/"dap_an_cuoi" PHẢI là hệ quả
TRỰC TIẾP của phép tính thật trong "loi_giai_chi_tiet" và "bieu_thuc_ket_qua" — TUYỆT ĐỐI không
chọn theo thói quen/khuôn mẫu (không mặc định "A", không mặc định xen kẽ Đúng-Sai-Đúng-Sai cho
TNDS — 4 ý ĐỘC LẬP, kết quả có thể là bất kỳ tổ hợp nào tùy phép tính thực tế, kể cả cả 4 ý cùng
Đúng hoặc cùng Sai). Nếu đáp án và lời giải không khớp nhau, sửa lại đáp án cho đúng với lời
giải (lời giải là nguồn sự thật).
...
#
# QUAN TRỌNG — không được để giá trị THẬT nào lọt vào các mẫu dưới đây (kể cả placeholder
# nhìn "hợp lý"), CHỈ dùng placeholder dạng hướng dẫn <...>. Từng có sự cố (v137): mẫu TNDS
# viết sẵn "Dung, Sai, Dung, Sai" và mẫu TN4PA viết sẵn "dap_an_dung": "A" — LLM (đặc biệt
# model nhẹ) có xu hướng CHÉP NGUYÊN khuôn few-shot này thay vì tự giải, trong khi
# "loi_giai_chi_tiet" (văn xuôi tự do, không có mẫu xen kẽ nào để chép) vẫn được suy luận
# đúng — kết quả: lời giải đúng nhưng đáp án trắc nghiệm/đúng-sai sai lệch. Đây là lỗi
# "nhiễm mẫu few-shot" (few-shot leakage), không phải lỗi cú pháp JSON.
_MAU_TN4PA = """{
  "loai_cau": "TN4PA",
  "de_bai": "<đề, công thức trong $...$>",
  "meta": {
    "phuong_an": {"A": "$...$", "B": "$...$", "C": "$...$", "D": "$...$"},
    "dap_an_dung": "<A/B/C/D — TỰ GIẢI để xác định, KHÔNG copy ví dụ>",
...
_MAU_TNDS = """{
  ...
  "de_bai": "<đề chung>",
  "meta": {"y": [
    {"ky_hieu": "a", "noi_dung_y": "$...$", "dap_an": "<Dung hoặc Sai — TỰ GIẢI ý này, KHÔNG copy ví dụ>", "bat_buoc_suy_luan": true},
    {"ky_hieu": "b", "noi_dung_y": "$...$", "dap_an": "<Dung hoặc Sai — TỰ GIẢI ý này>"},
    {"ky_hieu": "c", "noi_dung_y": "$...$", "dap_an": "<Dung hoặc Sai — TỰ GIẢI ý này>"},
    {"ky_hieu": "d", "noi_dung_y": "$...$", "dap_an": "<Dung hoặc Sai — TỰ GIẢI ý này>"}
  ]},
  "solution_steps": [
...
_MAU_TLN = """{
  "loai_cau": "TLN",
  "de_bai": "<đề>",
  "meta": {"dap_an_cuoi": "<giá trị số — TỰ TÍNH, KHÔNG copy ví dụ>", "quy_tac_lam_tron": null, "don_vi": null},
  "solution_steps": [
...
1. TỰ GIẢI bài toán bằng suy luận toán học chính xác của chính bạn để xác định đáp án đúng —
   BẮT BUỘC giải THẬT từng ý bằng phép tính/lập luận cụ thể, TUYỆT ĐỐI không đoán hay chọn
   theo thói quen/khuôn mẫu (vd không mặc định chọn "A", không mặc định xen kẽ Đúng-Sai-Đúng-Sai):
   - TN4PA: xác định phương án đúng ("dap_an_dung": "A"/"B"/"C"/"D").
   - TNDS: xác định Đúng/Sai cho TỪNG ý trong 4 ý đã cho ("dap_an": "Dung"/"Sai") — 4 ý ĐỘC LẬP
     với nhau, kết quả ngẫu nhiên có thể là bất kỳ tổ hợp nào (kể cả cả 4 ý cùng Đúng, hoặc
     cùng Sai) tùy vào phép tính thực tế của CHÍNH ý đó, KHÔNG suy ra từ vị trí a/b/c/d.
   - TLN: tính ra giá trị đáp án cuối cùng ("dap_an_cuoi").
   SAU KHI xác định "dap_an"/"dap_an_dung"/"dap_an_cuoi", BẮT BUỘC tự đối chiếu lại: giá trị
   đó phải là HỆ QUẢ TRỰC TIẾP của phép tính/lập luận trong "loi_giai_chi_tiet" và
   "bieu_thuc_ket_qua" ở mục 2 — nếu hai nơi không khớp nhau thì phải sửa lại "dap_an" cho
   đúng với lời giải (lời giải là nguồn sự thật, KHÔNG được sửa ngược lời giải để khớp đáp án
   đã trót chọn).
